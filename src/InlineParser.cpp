...
std::unique_ptr<InlineNode> InlineParser::parseInline() {
    ...
    if (c == '\n') {
        ...
        if (!nodes_.empty() && nodes_.back()->type == InlineType::Text) {
            ...
            if (lit.size() >= 2 && lit[lit.size()-1] == ' ' && lit[lit.size()-2] == ' ') {
    ...
    if (c == '!') {
        if (pos_ + 1 < input_.size() && input_[pos_ + 1] == '[') {
            ...
            if (!brackets_.empty()) brackets_.back().node = node.get();
            return node;
        }
    ...
    if (c == '<') {
        if (auto n = parseAutolink())   return n;
        if (auto n = parseHtmlInline()) return n;
        ++pos_;
        auto node     = makeNode(InlineType::Text);
        node->literal = "<";
        return node;
...
std::unique_ptr<InlineNode> InlineParser::parseBacktickString() {
    ...
    while (search < input_.size()) {
        if (input_[search] == '`') {
            ...
            while (search < input_.size() && input_[search] == '`') ++search;
...
std::unique_ptr<InlineNode> InlineParser::parseAutolink() {
    ...
    std::size_t sp = pos_;
    if (sp < input_.size() && std::isalpha(static_cast<unsigned char>(input_[sp]))) {
    ...
    pos_ = inner;
    std::size_t at = pos_;
    while (at < input_.size() && input_[at] != '@' && input_[at] != '>' && input_[at] != '\n')
        ++at;
    if (at < input_.size() && input_[at] == '@' && at > pos_) {
        ...
        if (local_ok) {
            ...
            while (dp < input_.size() && input_[dp] != '>' && input_[dp] != '\n') {
                char ch = input_[dp];
                if (ch == '.') {
                    if (dp == lbl) { domain_ok = false; break; }
                    lbl = dp + 1;
                } else if (!std::isalnum(static_cast<unsigned char>(ch)) && ch != '-') {
                    domain_ok = false; break;
                }
                ++dp;
            }
            if (domain_ok && dp < input_.size() && input_[dp] == '>' &&
                dp > at + 1 && dp != lbl) {
...
std::unique_ptr<InlineNode> InlineParser::parseHtmlInline() {
    ...
    if (pos_ >= input_.size()) { pos_ = save; return nullptr; }
    char c = input_[pos_];
    ...
    if (c == '/') {
        ...
        if (pos_ >= input_.size() || !std::isalpha(static_cast<unsigned char>(input_[pos_])))
            { pos_ = save; return nullptr; }
        while (pos_ < input_.size() && (std::isalnum(static_cast<unsigned char>(input_[pos_])) ||
               input_[pos_] == '-')) ++pos_;
        while (pos_ < input_.size() && input_[pos_] == ' ') ++pos_;
        if (pos_ < input_.size() && input_[pos_] == '>') { ++pos_; return emit(); }
        pos_ = save; return nullptr;
    }
    ...
    if (c == '!') {
        // HTML comment: <!--...-->
        if (pos_ + 2 < input_.size() && input_[pos_+1] == '-' && input_[pos_+2] == '-') {
            pos_ += 3;
            // must not start with > or ->
            if (pos_ < input_.size() && input_[pos_] == '>') { pos_ = save; return nullptr; }
            ...
            while (pos_ < input_.size()) {
                if (pos_+1 < input_.size() && input_[pos_] == '-' && input_[pos_+1] == '-') {
                    if (pos_+2 < input_.size() && input_[pos_+2] == '>')
                        { pos_ += 3; return emit(); }
                    pos_ = save; return nullptr; // -- not followed by >
                }
                ++pos_;
            }
            pos_ = save; return nullptr;
        ...
        // Declaration: <!NAME ...>
        if (pos_+1 < input_.size() && std::isupper(static_cast<unsigned char>(input_[pos_+1]))) {
            ++pos_; // skip '!'
            while (pos_ < input_.size() && std::isupper(static_cast<unsigned char>(input_[pos_])))
                ++pos_;
    ...
    if (c == '?') {
        ...
        while (pos_+1 < input_.size()) {
            if (input_[pos_] == '?' && input_[pos_+1] == '>') { pos_ += 2; return emit(); }
            ++pos_;
        }
        pos_ = save; return nullptr;
    }
    ...
    if (!std::isalpha(static_cast<unsigned char>(c))) { pos_ = save; return nullptr; }

    while (pos_ < input_.size() &&
           (std::isalnum(static_cast<unsigned char>(input_[pos_])) || input_[pos_] == '-'))
        ++pos_;
    ...
    if (input_[pos_] == '>') { ++pos_; return emit(); }
    pos_ = save; return nullptr;
}
...
std::optional<std::string> InlineParser::scanLinkDestination() {
    ...
    if (input_[pos_] == '<') {
        ...
        while (pos_ < input_.size() && input_[pos_] != '\n') {
            char c = input_[pos_];
            if (c == '>') { ++pos_; return dest; }
            if (c == '<') { pos_ = save; return std::nullopt; }
            if (c == '\\' && pos_+1 < input_.size() && isAsciiPunct(input_[pos_+1]))
                { dest += input_[pos_+1]; pos_ += 2; }
            else { dest += c; ++pos_; }
        }
        pos_ = save; return std::nullopt;
    }
    ...
    while (pos_ < input_.size()) {
        char c = input_[pos_];
        if (static_cast<unsigned char>(c) <= 0x1F || c == ' ') break;
        if (c == '(') {
            ++depth;
        } else if (c == ')') {
            if (depth == 0) break;
            --depth;
        } else if (c == '\\' && pos_+1 < input_.size() && isAsciiPunct(input_[pos_+1])) {
            dest += input_[pos_+1]; pos_ += 2; continue;
        }
        dest += c;
        ++pos_;
    }
    if (depth != 0) { pos_ = save; return std::nullopt; }
    return dest;
}
...
std::optional<std::string> InlineParser::scanLinkTitle() {
    ...
    while (pos_ < input_.size()) {
        char c = input_[pos_];
        if (c == close) { ++pos_; return title; }
        if (open == '(' && c == '(') { pos_ = save; return std::nullopt; }
        if (c == '\\' && pos_+1 < input_.size() && isAsciiPunct(input_[pos_+1]))
            { title += input_[pos_+1]; pos_ += 2; }
        else { title += c; ++pos_; }
    }
    pos_ = save; return std::nullopt;
}
...
std::unique_ptr<InlineNode> InlineParser::handleBracketCloser() {
    ...
    if (brackets_.empty()) return literal_bracket();
    ...
    auto get_bracket_text = [&]() -> std::string {
        auto op_it = std::find_if(nodes_.begin(), nodes_.end(),
            [&](const auto& p) { return p.get() == bracket.node; });
        std::string text;
        if (op_it == nodes_.end()) return text;
    ...
    if (!resolved && pos_ < input_.size() && input_[pos_] == '(') {
        ...
        if (!resolved) { pos_ = il_save; dest = std::nullopt; title = std::nullopt; }
    }
    ...
    if (!resolved && pos_ < input_.size() && input_[pos_] == '[') {
        ...
        if (!resolved) pos_ = fr_save;
    }

    // 3. Collapsed reference: ][]
    if (!resolved && pos_+1 < input_.size() && input_[pos_] == '[' && input_[pos_+1] == ']') {
    ...
    if (resolved) {
        ...
        auto link  = makeNode(bracket.is_image ? InlineType::Image : InlineType::Link);
        ...
        // Links cannot be nested: deactivate any preceding link (not image) openers.
        if (!bracket.is_image) {
            for (auto& b : brackets_)
                if (!b.is_image) b.node = nullptr;
        }
...
void InlineParser::handleEmphasis(char delim_char, std::size_t run_len) {
    ...
    if (!can_open && !can_close) return; // literal text, no delimiter pushed
...
void InlineParser::processEmphasis(std::optional<std::size_t> stack_bottom) {
    ...
    while (ci < delimiters_.size()) {
        ...
        if (!closer.can_close) { ++ci; continue; }
        ...
        while (opIdx > ob[oi(c)]) {
            ...
            if (!opener.can_open || opener.ch != c) continue;
            ...
            if ((opener.can_open && opener.can_close) ||
                ...
                if ((opener.num + delimiters_[ci].num) % 3 == 0 &&
                    opener.num % 3 != 0)
                    continue;
            }
            found = true;
        ...
        auto cl_it = std::find_if(nodes_.begin(), nodes_.end(),
