...
static const SimpleFold kSimpleFolds[1171] = {
  {0x0041, 0x0061},
  {0x0042, 0x0062},
  {0x0043, 0x0063},
  {0x0044, 0x0064},
...
struct MultiFold { uint32_t from; const char* to; };
static const MultiFold kMultiFolds[104] = {
  {0x00DF, "\x73\x73"},
  {0x0130, "\x69\xCC\x87"},
  {0x0149, "\xCA\xBC\x6E"},
  {0x01F0, "\x6A\xCC\x8C"},
...
// Decode one UTF-8 codepoint from s[pos]; advance pos. Returns 0xFFFD on error.
static uint32_t decodeUtf8(std::string_view s, std::size_t &pos) {
  auto b = [&](std::size_t i) -> uint8_t {
    return static_cast<uint8_t>(s[i]);
  };
  uint8_t c0 = b(pos);
  if (c0 < 0x80) { ++pos; return c0; }
  if (c0 < 0xC2 || c0 > 0xF4) { ++pos; return 0xFFFD; }
  std::size_t extra = (c0 >= 0xF0) ? 3 : (c0 >= 0xE0) ? 2 : 1;
  if (pos + extra >= s.size()) { ++pos; return 0xFFFD; }
  uint32_t cp = c0 & (0x7F >> extra);
  for (std::size_t i = 1; i <= extra; ++i) {
    uint8_t cb = b(pos + i);
    if ((cb & 0xC0) != 0x80) { ++pos; return 0xFFFD; }
    cp = (cp << 6) | (cb & 0x3F);
  }
  pos += extra + 1;
  return cp;
...
static std::string encodeUtf8(uint32_t cp) {
  ...
  if (cp < 0x800) return {static_cast<char>(0xC0|(cp>>6)), static_cast<char>(0x80|(cp&0x3F))};
  if (cp < 0x10000) return {static_cast<char>(0xE0|(cp>>12)), static_cast<char>(0x80|((cp>>6)&0x3F)), static_cast<char>(0x80|(cp&0x3F))};
  return {static_cast<char>(0xF0|(cp>>18)), static_cast<char>(0x80|((cp>>12)&0x3F)), static_cast<char>(0x80|((cp>>6)&0x3F)), static_cast<char>(0x80|(cp&0x3F))};
}
...
std::string foldString(std::string_view s) {
  ...
  std::size_t pos = 0;
  while (pos < s.size()) {
    uint32_t cp = decodeUtf8(s, pos);
    // Check multi-char folds first (they take precedence).
    {
      auto it = std::lower_bound(kMultiFolds, kMultiFolds + 104,
          cp, [](const MultiFold &e, uint32_t v){ return e.from < v; });
      if (it != kMultiFolds + 104 && it->from == cp) {
        out += it->to;
        continue;
      }
    }
    // Check simple folds.
    {
      auto it = std::lower_bound(kSimpleFolds, kSimpleFolds + 1171,
          cp, [](const SimpleFold &e, uint32_t v){ return e.from < v; });
      if (it != kSimpleFolds + 1171 && it->from == cp) {
        out += encodeUtf8(it->to);
        continue;
      }
    }
    out += encodeUtf8(cp);
