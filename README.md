...
The suite (~689 tests) runs under CTest:
...
It comprises two executables:

- **`markdown_tests`** — the complete CommonMark v0.31.2 spec conformance suite.
  Every spec example is loaded from
  [`tests/test-files/markdown/commonmark_spec.json`](tests/test-files/markdown/commonmark_spec.json)
  and run as an individual parameterized test case (source in
  [`tests/markdown/`](tests/markdown/)).
- **`mermaid_tests`** — lexer/parse/lower unit tests plus **golden-file
  ...
  official Mermaid library (fixtures in
  [`tests/test-files/mermaid/`](tests/test-files/mermaid/); see
  [`mermaid-utils/`](mermaid-utils/README.md) for how goldens are produced).

Run a subset, e.g. only the CommonMark spec suite, or only the Mermaid AST
verification:

```bash
cd build && ctest -L spec --output-on-failure     # markdown spec conformance
cd build && ctest -R FlowAst --output-on-failure  # mermaid AST goldens
```
